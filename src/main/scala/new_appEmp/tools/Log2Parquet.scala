...
import new_appEmp.utils.{LogSchema, NumFormat}
...
object Log2Parquet {
  ...
  //1、引入环境
    ...
    val value = txt.map(_.split(",", -1)).filter(_.length >= 85)
    val roww :RDD[Row] = value.map(arr => {
        Row(
          arr(0),
    ...
    val df = spark.createDataFrame(roww, LogSchema.stuctType)
